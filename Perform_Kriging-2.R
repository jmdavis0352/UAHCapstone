...
Kriged<-foreach(f = 1:15, .combine = rbind, .packages = c("lhs", "geoR", "ncdf4", "foreach"),
  ...
  GEFSfile<-nc_open(paste("Source Data/train_independent/", trainfiles[f], sep = ""))
  ...
  dimStation<-ncdim_def(name ="Station", units ="Name", vals = 1:98)
  ...
  krigGEFfile<-nc_create(paste0("E:/Dropbox/Kaggle/Solar Energy/R_ProjectSolarEnergy/Derived Data/KrigedTrainingSE/train_Krig_",VarName, ".nc"), list(krigGEFSVAR,krigGEFSVARSE) )
  
  ##  loop over time of day 3-5, and times 1:5113 to create kriging predictions at the Mesonet locations
  Results<-foreach(d= 1:5, .combine = rbind, .errorhandling = 'remove') %:% foreach(t = 1:5113, .combine = rbind, .verbose = FALSE, .errorhandling = 'remove') %do% {
    print(paste(f,d, t))
